...
Vmax = 100000; costbin = 8000; %test different bin sizes - would need to vary cost accordingly as well
costmaintenance = 5000;%scale down maintenance costs to the bin level - need to vary maintenance cost and frequency of trash collection together
...
budget = 2000000; %hypothetically let's say budget is $1,000,000 USD for maintaining trash in Harvard Square
...
xlabel('time')
ylabel('People')
title(['Number of people per minute over a ' num2str(tend) 'minute period'])

nexttile %plot the number of litter items per minute vs time
plot(time,L)
xlabel('time')
ylabel('Litter (items)')
title(['Litter items generated per minute over a ' num2str(tend) 'minute period'])

nexttile %plot the amount of overflow trash per minute
plot(time,overflow)
xlabel('time')
ylabel('Overflow (items)')
title(['Overflow items generated per minute over a ' num2str(tend) 'minute period'])