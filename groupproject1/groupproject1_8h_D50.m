...
format longg
%K = 20; %bacterial carrying capacity
%tend = 1440; %time between doses = 8 hours. 8*60 = 480 minutes
%dt = 1; %timestep = 1 minute
...
g = 1/10; %1, 10, 100 # of bacterial cells killed per unit drug per minute
D1 = 50; %use molecules of amoxicillin - units need to match that of bacteria. 
          %initial drug concentration concides with peak plasma concentration for intravenous injection = 4.75 ug/mL for 250 mg dose
...
l = 0; %before add in for loop
    ...
    if y1(length(y1(:,1)),1) < 0 %if drug concentration fell below zero
        ...
        D1 = y1(length(y1(:,1)),1) + D1; %where we left off + new dose (D1)
    end 
    ...
    tdBdt2 = horzcat((481:1:960)',dBdt2);
    ...
    if y2(length(y2(:,1)),1) < 0 %if drug concentration fell below zero
        ...
        D1 = y2(length(y2(:,1)),1) + D1; %where we left off + new dose (D1)
    end 
    ...
    if y3(length(y3(:,1)),1) < 0 %if drug concentration fell below zero
        ...
        D1 = y3(length(y3(:,1)),1) + D1; %where we left off + new dose (D1)
    end 
    ...
    if y4(length(y4(:,1)),1) < 0 %if drug concentration fell below zero
        ...
        D1 = y4(length(y4(:,1)),1) + D1; %where we left off + new dose (D1)
    end 
